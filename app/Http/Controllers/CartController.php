...
class CartController extends Controller
{
    ...
    public function confirm_order_details(Request $request)
    {
        ...
        foreach ($cart_res as $n => $cart_item) {
            ...
            $item_names[] = $cart_item->item_name;
            $item_prices[] = $cart_item->item_price;
        }

        $checkout_order = new OrderCheckout;
        $checkout_order->user_id = $user_id;
        $checkout_order->item_name = json_encode($item_names);
        $checkout_order->item_price = json_encode($item_prices);
        if($checkout_order->save()){
            Cart::where('user_id', $user_id)->delete();
            $id = $checkout_order->id;
            return redirect()->route('view.checkout', $id);
        }
...
//        return view('pages.cart_order_confirm_details');
    ...
    public function view_checkout($cart_id)
    {
        $order = OrderCheckout::find($cart_id);
